...
function daemonPaths(view2) {
  return {
    daemonLog: join(homedir(), ".hindsight", "profiles", `${view2.daemonProfile}.log`),
    pluginLog: process.env.HINDSIGHT_LOG_FILE || join(tmpdir(), "hindsight-coding-agent", "plugin.log"),
    database: join(homedir(), ".pg0", "instances", `hindsight-embed-${view2.daemonProfile}`)
  };
}
...
function findDaemonStarter(startDir = dirname(fileURLToPath(import.meta.url))) {
  ...
  for (const name2 of profileNames) {
    const candidate = join(profilesRoot, name2, "node_modules", ...rel);
    if (existsSync(candidate)) return candidate;
  }
  return null;
}
function embedCommand(view2) {
  if (view2.embedPackagePath) return { cmd: "uv", base: ["run", "--directory", view2.embedPackagePath, "hindsight-embed"] };
  const version = view2.embedVersion && view2.embedVersion.length > 0 ? view2.embedVersion : "latest";
...
async function stopDaemon(view2) {
  const steps = [];
  let output = "";
  let after = await checkHealth(view2.apiUrl);
  if (process.platform === "win32" && after.running) {
    const kill = await killPortProcess(view2.apiPort);
    steps.push("port-kill :" + view2.apiPort + " (pid " + (kill.killed || "none") + ")");
    output += kill.output;
    for (let i = 0; i < 10; i++) {
      await new Promise((r2) => setTimeout(r2, 500));
      after = await checkHealth(view2.apiUrl);
      if (!after.running) break;
    }
  }
  if (after.running) {
    const { cmd, base } = embedCommand(view2);
    const args = [...base, "daemon", "--profile", view2.daemonProfile, "stop"];
...
function startDaemon(view2, harness, starter) {
  if (starter) {
  ...
  const { cmd, base } = embedCommand(view2);
  const args = [...base, "daemon", "--profile", view2.daemonProfile, "start"];
...
// src/index.ts
var name = "dsh-hindsight-manager";
var inject = ["webServer"];
var API_PREFIX = "/dsh-hindsight-manager/api";
var HARNESS = "dsh";
